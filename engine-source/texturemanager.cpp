...
SDL_Texture* TextureManager::LoadTextureTextTran(TTF_Font* _font, int _width, std::string _text, SDL_Color _color, SDL_Renderer* _ren){
	SDL_Surface* tempSurface = TTF_RenderText_Blended_Wrapped(_font, _text.c_str(), _color, _width);
	SDL_Texture* tex = SDL_CreateTextureFromSurface(_ren, tempSurface);
	SDL_FreeSurface(tempSurface);
